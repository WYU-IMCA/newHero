...
namespace imca::serial_driver
{
  ...
  public:
    ...
    static std::unique_ptr<protocol::Protocol> createProtocol(std::string_view protocol_type,
                                                              ...
                                                              bool enable_data_print, bool enable_send_data_print)
    {
      if (protocol_type == "infantry")
      {
        return std::make_unique<protocol::ProtocolInfantry>(port_name, enable_data_print, enable_send_data_print);
      }
      if (protocol_type == "hero")
      {
        return std::make_unique<protocol::DefaultProtocol>(port_name, enable_data_print, enable_send_data_print);
      }
      ...
      {
        return std::make_unique<protocol::ProtocolSentry>(port_name, enable_data_print, enable_send_data_print);
      }
