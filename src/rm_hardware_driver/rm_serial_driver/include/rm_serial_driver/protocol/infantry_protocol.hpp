...
namespace imca::serial_driver::protocol
{
  ...
  {
  public:
    explicit ProtocolInfantry(std::string_view port_name, bool enable_data_print, bool enable_send_data_print);
    ...
    std::vector<rclcpp::AsyncParametersClient::SharedPtr> getParamClients(
        rclcpp::Node::SharedPtr node) override;
