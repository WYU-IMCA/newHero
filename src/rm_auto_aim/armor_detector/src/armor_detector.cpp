...
std::vector<Armor> Detector::detect(const cv::Mat &input,const bool use_numberclass) noexcept {
  ...
  if (!armors_.empty() && classifier != nullptr) {
    // Parallel processing
    std::for_each(
      std::execution::par, armors_.begin(), armors_.end(), [this, &input, use_numberclass](Armor &armor) {
  ...
  if(!use_numberclass){
    armors_.erase(
      std::remove_if(armors_.begin(),armors_.end(),
