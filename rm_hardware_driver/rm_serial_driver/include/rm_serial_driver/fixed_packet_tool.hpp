...
namespace fyt::serial_driver
{
  ...
  public:
    ...
    void enbaleSendDataPrint(bool enable) { use_send_data_print_ = enable; }
  ...
  private:
    ...
    bool use_send_data_print_{false};
  ...
  bool FixedPacketTool<capacity>::checkPacket(uint8_t *buffer, int recv_len)
  {
    ...
    // 检查帧头，帧尾,
    if ((buffer[0] != 0x5a) || (buffer[capacity - 1] != 0xa5))
    {
      return false;
    }
